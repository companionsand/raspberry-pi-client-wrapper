...
echo "=========================================="
echo "📊 ALSA Loopback Setup for Speaker Monitor"
echo "=========================================="

# Check if running as root
if [ "$EUID" -ne 0 ]; then
    echo "❌ Please run as root (sudo ./install-loopback.sh)"
    exit 1
fi
...
echo ""
echo "1️⃣  Loading snd-aloop kernel module..."
...
if ! grep -q "snd-aloop" /etc/modules-load.d/*.conf 2>/dev/null; then
    ...
    echo "   ✓ Added to /etc/modules-load.d/snd-aloop.conf (persistent)"
else
    echo "   ✓ Already in modules-load.d (persistent)"
fi

echo ""
echo "2️⃣  Creating ALSA configuration..."
...
if [ -z "$RESPEAKER_CARD" ]; then
    echo "   ⚠️  ReSpeaker not found, using 'default' for playback"
...
if [ -z "$LOOPBACK_CARD" ]; then
    echo "   ❌ Loopback device not found. Is snd-aloop loaded?"
    exit 1
fi
...
if aplay -l 2>/dev/null | grep -q "Loopback"; then
    echo "   ✓ Loopback device accessible"
else
    echo "   ⚠️  Loopback device not showing in aplay -l"
fi

# Test that speaker_monitor is readable
if arecord -L 2>/dev/null | grep -q "speaker_monitor"; then
    echo "   ✓ speaker_monitor device configured"
else
    echo "   ⚠️  speaker_monitor not in arecord -L (may need alsactl restore)"
fi

echo ""
echo "=========================================="
echo "✅ ALSA Loopback Setup Complete!"
echo "=========================================="
echo ""
...
echo "  • speaker_monitor  - Read from this to capture speaker output"
echo "  • loopback_out     - Loopback playback (internal)"
echo "  • speaker_with_monitor - Multi-output (speaker + loopback)"
echo ""
